...
{
    [Header("Cesium Georefernce")]
    public CesiumGeoreference georeference;

    [Header("Floor Prefab")]
    [Tooltip("Floor by GPS")]
    public GameObject floorPrefab;

    [Header("Player Anchor")]
    public Camera playerCamera;
    private CesiumGlobeAnchor playerAnchor;
    private CesiumGlobeAnchor floorAnchor;
    ...
    IEnumerator Start()
    {
        ...
        if (floorPrefab != null)
        {
            GameObject newFloor = Instantiate(floorPrefab, Vector3.zero, Quaternion.identity);
            newFloor.transform.SetParent(georeference.transform, false);
            CesiumGlobeAnchor floorAnchor = newFloor.AddComponent<CesiumGlobeAnchor>();
            floorAnchor.longitudeLatitudeHeight = new double3(longitude, latitude, 0);

            Debug.Log("Create Floor");
        }
        else
        {
            Debug.LogWarning("Floor Prefab이 할당되지 않아 생성하지 않았습니다.");
        }
